...
export class EightDError extends Error {
  constructor(
    public readonly code:
      ...
      | "ROOT_CAUSE_REQUIRED"
      | "CAPA_APPROVAL_REQUIRED"
      | "CAPA_VALIDATION_REQUIRED"
      | "PREVENTION_REQUIRED"
...
function parseEvent(value: string | null): EventReference | null {
  const parsed = parseJson<Partial<EventReference>>(value);
  if (
    !parsed ||
...
function parseRootCause(value: string | null): RootCauseReference | null {
  const parsed = parseJson<Partial<RootCauseReference>>(value);
  if (
    !parsed ||
    typeof parsed.organizationId !== "string" ||
    typeof parsed.siteId !== "string" ||
...
function parseCapa(value: string | null): CapaReference | null {
  const parsed = parseJson<Partial<CapaReference>>(value);
  if (
    !parsed ||
    typeof parsed.organizationId !== "string" ||
    typeof parsed.siteId !== "string" ||
...
function parseSnapshot(value: string | null): EightDSnapshot | null {
  const parsed = parseJson<Partial<EightDSnapshot>>(value);
  if (
    !parsed ||
...
async function latestSnapshot<T>(
...
async function ownerNames(tx: Prisma.TransactionClient, ownerIds: string[]) {
  const users = await tx.user.findMany({
    where: { id: { in: [...new Set(ownerIds)] } },
...
export async function saveEightDWorkspace(input: {
  ...
  return serializable(async (tx) => {
    ...
    const snapshot: EightDSnapshot = {
      ...
      d6Validation: previous?.d6Validation ?? null,
    ...
    switch (discipline) {
      ...
      case "D5": {
        ...
        if (
          ...
          throw new EightDError("CAPA_APPROVAL_REQUIRED", "D5 requires an approved CAPA with permanent actions");
        }
        const names = await ownerNames(tx, capa.actions.map((action) => action.ownerId));
        snapshot = {
          ...snapshot,
      ...
      case "D6": {
        const capa = await latestCapa(tx, input.eventId);
        const latestEffectiveness = capa?.effectivenessChecks.at(-1) ?? null;
        if (
          !capa ||
          capa.organizationId !== input.organizationId ||
          capa.siteId !== input.siteId ||
          ...
          !latestEffectiveness ||
          latestEffectiveness.result !== "EFFECTIVE" ||
          !latestEffectiveness.note.trim()
        ) {
          throw new EightDError(
            "CAPA_VALIDATION_REQUIRED",
            "D6 requires all CAPA actions completed and effectiveness confirmed as effective",
          );
        }
        snapshot = {
          ...snapshot,
          d6Validation: {
            ...
            effectivenessNote: latestEffectiveness.note.trim(),
