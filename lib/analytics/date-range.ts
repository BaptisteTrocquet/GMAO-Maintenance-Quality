...
export function localDateStartUtc(value: string, timeZone: string) {
  ...
  // Two passes handle the common DST case where the initial UTC guess has a different offset
  // from local midnight. A final local-date check keeps failures explicit rather than silent.
...
export function resolveAnalyticsDateRange(input: {
  ...
  const toExclusive = input.to
    ? localDateStartUtc(nextCalendarDate(input.to), input.timeZone)
    : null;
