export const PLANNING_CALENDAR_DAYS = 14;
export const PLANNING_CALENDAR_LIMIT = 300;
...
export function calendarDateKeys(startDateKey: string, days = PLANNING_CALENDAR_DAYS) {
  if (!Number.isInteger(days) || days <= 0) throw new Error("Calendar days must be a positive integer");
...
export function zonedDateTimeToUtc(parts: ZonedDateTimeParts, timeZone: string) {
  ...
  // Iterating accounts for offsets that change near DST boundaries without adding a date library.
...
export function startOfDateKeyInTimeZone(dateKey: string, timeZone: string) {
  const parsed = parseDateKey(dateKey);
  if (!parsed) throw new Error(`Invalid date key: ${dateKey}`);
...
export function shiftInstantByLocalDays(date: Date, days: number, timeZone: string) {
  ...
  const originalKey = dateKeyFromParts(original);
  const shiftedKey = addDaysToDateKey(originalKey, days);
...
export function rescheduleWorkOrderForDate(input: {
  ...
  return {
    ...
    dueAt: input.dueAt
      ? shiftInstantByLocalDays(input.dueAt, deltaDays, input.timeZone)
      : null,
  };
}
