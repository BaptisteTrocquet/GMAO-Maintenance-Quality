...
const saveSchema = scopeSchema.extend({
  ...
  impactScope: z.string().trim().max(5000).optional(),
  escapePoint: z.string().trim().max(5000).optional(),
  validationNote: z.string().trim().max(5000).optional(),
...
const transitionSchema = z.discriminatedUnion("action", [
  scopeSchema.extend({ action: z.literal("ADVANCE") }),
  scopeSchema.extend({ action: z.literal("RESET_AFTER_INEFFECTIVE_CAPA") }),
]);
  ...
  try {
    ...
    return apiData(
      parsed.data.action === "ADVANCE"
        ? await advanceEightD(common)
        : await resetEightDAfterIneffectiveCapa(common),
    );
  } catch (error) {
    if (error instanceof EightDError) return eightDError(error);
    throw error;
