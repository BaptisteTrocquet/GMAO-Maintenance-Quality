...
const saveSchema = scopeSchema.extend({
  ...
  systemicChanges: z.array(z.string().trim().min(1).max(2000)).max(50).optional(),
  recognitionNote: z.string().trim().max(5000).optional(),
});

const advanceSchema = scopeSchema.extend({ action: z.literal("ADVANCE") });
...
export async function PUT(
  ...
  if (!parsed.success) {
    return apiError(400, "INVALID_PAYLOAD", "Invalid 8D workspace payload", parsed.error.flatten());
  }
