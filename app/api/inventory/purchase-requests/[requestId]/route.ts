...
const patchSchema = z
  ...
  .superRefine((value, ctx) => {
    const hasDraftChange =
      value.reason !== undefined || value.neededBy !== undefined || value.lines !== undefined;
    if (value.action && hasDraftChange) {
      ctx.addIssue({
        code: "custom",
        message: "Status transitions cannot modify draft fields",
      });
    }
    if (!value.action && !hasDraftChange) {
      ctx.addIssue({
        code: "custom",
        message: "Draft update requires reason, neededBy or lines",
      });
    }
  });
