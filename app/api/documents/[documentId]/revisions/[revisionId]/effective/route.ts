...
import { DocumentWorkflowError, scheduleRevisionEffective } from "@/lib/documents/workflow";
...
export async function POST(
  ...
  if (!parsed.success) return apiError(400, "INVALID_PAYLOAD", "Invalid effective-date request", parsed.error.flatten());
  ...
  try {
    return apiData(
      await scheduleRevisionEffective({
