...
import { buildPartsCostDashboard, PartsCostAnalyticsError } from "@/lib/analytics/parts-cost";
...
export async function GET(request: Request): Promise<Response> {
  ...
  if (!can(auth.tenant.scope.role, "inventory:read")) {
    return apiError(403, "ACCESS_DENIED", "Inventory analytics permission required");
  }
  ...
  try {
    ...
    if (error instanceof PartsCostAnalyticsError) {
