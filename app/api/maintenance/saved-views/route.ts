...
const createSchema = z.discriminatedUnion("surface", [
  ...
  scopeSchema.extend({
    ...
    config: z.object({ month: monthSchema.nullable() }),
  }),
]);
...
export async function GET(request: Request) {
  ...
  const surfaceParsed = surfaceValue ? surfaceSchema.safeParse(surfaceValue) : null;
  if (surfaceValue && !surfaceParsed?.success) {
    return apiError(400, "INVALID_SURFACE", "Unsupported saved-view surface");
  }
  ...
  return apiData(
    await listSavedMaintenanceViews({
      ...
      surface: surfaceParsed?.success ? (surfaceParsed.data as SavedMaintenanceSurface) : undefined,
    }),
  );
}
