  ...
  if (!organizationId || !siteId) {
    return (
      <section className="card">
        <p>Select an organization and site to open 8D.</p>
      </section>
    );
  }
          ...
          <ol className="timeline">
            {timeline.map((entry) => (
              <li key={entry.id}>
                ...
                <span>
                  {entry.actorName}
                  {entry.after ? ` · v${entry.after.version} · ${entry.after.currentDiscipline}` : ""}
                </span>
              </li>
            ))}
          </ol>
        ) : (
          <p className="muted">No 8D activity recorded yet.</p>
        )}
      </section>
    </>
  );
