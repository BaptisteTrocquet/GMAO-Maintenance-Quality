...
export default async function QualityEvidencePage({
  ...
  const [qualityEvent, evidence] = await Promise.all([
    ...
    listQualityEvidence({ organizationId, siteId, eventId, includeRevoked: true }),
  ]);
    ...
    <>
      <div className="header asset-header">
        ...
        <div className="asset-status">
          ...
          <span className="badge">{evidence.filter((item) => item.active).length} ACTIVE</span>
          <span className="badge">{evidence.filter((item) => !item.active).length} REVOKED</span>
        </div>
      </div>
