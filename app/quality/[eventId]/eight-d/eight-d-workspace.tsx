...
export default function EightDWorkspace({
  ...
  async function save() {
    return request("PUT", {
      team: team.map(({ userId, responsibility }) => ({ userId, responsibility })),
      problemStatement,
      impactScope,
      escapePoint,
      validationNote,
    ...
    <div className="grid" style={{ gap: 18 }}>
      ...
      <section className="card">
        <div className="header asset-header" style={{ marginBottom: 12 }}>
          ...
          {editable ? <button type="button" onClick={addTeamMember} style={buttonStyle}>+ Team member</button> : null}
        </div>
      ...
      <section className="card">
        ...
        <textarea value={problemStatement} onChange={(event) => setProblemStatement(event.target.value)} disabled={!editable || busy} rows={3} placeholder="Specific, observable problem statement" style={{ ...inputStyle, resize: "vertical" }} />
        <textarea value={impactScope} onChange={(event) => setImpactScope(event.target.value)} disabled={!editable || busy} rows={2} placeholder="Impact / scope / affected population" style={{ ...inputStyle, resize: "vertical", marginTop: 10 }} />
      </section>
      ...
      <section className="card">
        ...
        <textarea value={escapePoint} onChange={(event) => setEscapePoint(event.target.value)} disabled={!editable || busy} rows={2} placeholder="Where and why did the detection/control system allow the issue to escape?" style={{ ...inputStyle, resize: "vertical" }} />
      </section>
      ...
      <section className="card">
        ...
        <textarea value={validationNote} onChange={(event) => setValidationNote(event.target.value)} disabled={!editable || busy} rows={3} placeholder="Objective validation evidence for the implemented corrective actions" style={{ ...inputStyle, resize: "vertical" }} />
      ...
      <section className="card">
        ...
        <textarea value={preventionSummary} onChange={(event) => setPreventionSummary(event.target.value)} disabled={!editable || busy} rows={3} placeholder="How recurrence is prevented systemically" style={{ ...inputStyle, resize: "vertical" }} />
        <textarea value={systemicChanges} onChange={(event) => setSystemicChanges(event.target.value)} disabled={!editable || busy} rows={3} placeholder="One systemic change per line" style={{ ...inputStyle, resize: "vertical", marginTop: 10 }} />
      </section>

      <section className="card">
        <h2>D8 · Close & recognize</h2>
        <textarea value={recognitionNote} onChange={(event) => setRecognitionNote(event.target.value)} disabled={!editable || busy} rows={2} placeholder="Team recognition / closure note" style={{ ...inputStyle, resize: "vertical" }} />
        <textarea value={lessonsLearned} onChange={(event) => setLessonsLearned(event.target.value)} disabled={!editable || busy} rows={3} placeholder="Lessons learned and knowledge to retain" style={{ ...inputStyle, resize: "vertical", marginTop: 10 }} />
      </section>

      <section className="card">
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {editable ? <button type="button" disabled={busy} onClick={save} style={buttonStyle}>Save 8D</button> : null}
          {editable ? <button type="button" disabled={busy} onClick={advance} style={buttonStyle}>Complete {eightD?.currentDiscipline ?? "D1"} →</button> : null}
