...
export default function MttrClient({
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const params = new URLSearchParams({ organizationId, siteId, from: fromDay, to: throughDay });
      if (assetId) params.set("assetId", assetId);

      const response = await fetch(`/api/analytics/mttr?${params.toString()}`, { cache: "no-store" });
    ...
    <>
      <section className="card">
        ...
        <p className="muted" style={{ marginBottom: 0 }}>
          Definition: completed CORRECTIVE work orders whose completion timestamp falls on the selected local calendar dates. Repair time is completedAt minus startedAt. Missing or reversed timestamps are excluded from the mean and reported separately.
        </p>
      </section>
          ...
          <div className="grid grid-2 section">
            <section className="card">
              ...
              {data.empty ? (
                ...
                <p className="muted">
                  Completed corrective work exists, but none has a valid startedAt → completedAt interval. MTTR is therefore undefined rather than zero.
                </p>
              ) : (
