      ...
      <section className="card" style={{ marginBottom: 16 }}>
        ...
        {payload ? (
          <div className="muted" style={{ marginTop: 8 }}>
            Generated {new Date(payload.generatedAt).toLocaleString(undefined, { timeZone: payload.timezone })} · {payload.timezone}
          </div>
        ) : null}
      </section>
          ...
          <div className="grid grid-2 section">
            ...
            <section className="card responsive-table">
              ...
              <table className="table">
                ...
                <tbody>
                  <tr><td>0–6 calendar days</td><td>{payload.aging.DAYS_0_6}</td></tr>
                  <tr><td>7–29 calendar days</td><td>{payload.aging.DAYS_7_29}</td></tr>
                  <tr><td>30–89 calendar days</td><td>{payload.aging.DAYS_30_89}</td></tr>
                  <tr><td>90+ calendar days</td><td>{payload.aging.DAYS_90_PLUS}</td></tr>
                </tbody>
              </table>
            </section>
          </div>
                ...
                <tbody>
                  {payload.oldest.map((workOrder) => (
                    <tr key={workOrder.id}>
                      ...
                      <td>{formatDate(workOrder.requestedAt, payload.timezone)}</td>
                      <td>{formatDate(workOrder.dueAt, payload.timezone)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
