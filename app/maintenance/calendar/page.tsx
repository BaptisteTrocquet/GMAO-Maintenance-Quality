...
import { PlanningCalendarDnd } from "./planning-calendar-dnd";
...
export default async function MaintenanceCalendarPage({
  ...
  const dragDays = calendar.map((day) => ({
    dateKey: day.dateKey,
  ...
  const dragUnscheduled = visibleUnscheduled.map((workOrder) => ({
    id: workOrder.id,
    number: workOrder.number,
    title: workOrder.title,
    status: workOrder.status,
    ...
    <>
      ...
      <section className="card" aria-label="Calendar month navigation">
        ...
        <p className="muted" style={{ marginBottom: 0 }}>
          Planned starts and due dates use the organization timezone. Drag a planned work order to another day to keep its local start time, or drag unscheduled work onto a day to plan it at 09:00 local time.
        </p>
        {truncated ? (
