...
export default function WorkOrderCard({ organizationId, siteId, workOrder }: Props) {
  ...
  return (
    <article className="kanban-card" aria-label={`${workOrder.number} ${workOrder.title}`}>
      <div className="kanban-card-header">
        <div>
          <Link className="table-link" href={`/maintenance/${workOrder.id}`}>
            <strong>{workOrder.number}</strong>
          </Link>
          <div className="kanban-card-title">{workOrder.title}</div>
        </div>
        <span className="badge">{workOrder.priority}</span>
      </div>

      <dl className="detail-list kanban-card-details">
        ...
        <div>
          <dt>Due</dt>
          <dd>
            {formatDate(workOrder.dueAt)}
            {workOrder.overdue ? <span className="kanban-overdue"> · OVERDUE</span> : null}
          </dd>
        </div>
      </dl>

      {targets.length ? (
        <div className="kanban-actions" aria-label="Available status transitions">
          {targets.map((target) => {
      ...
      {error ? <div className="kanban-error" role="alert">{error}</div> : null}
    </article>
  );
}
