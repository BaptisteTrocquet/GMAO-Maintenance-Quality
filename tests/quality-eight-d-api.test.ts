...
describe("quality 8D API", () => {
  ...
  it("lets viewers read the 8D workspace", async () => {
    ...
    const response = requireResponse(
      await GET(
        new Request(
          "http://localhost/api/quality/events/event-1/eight-d?organizationId=org-a&siteId=site-a",
        ),
        context,
      ),
    );
  ...
  it("lets quality managers save D1, D2, D7 and D8 data", async () => {
      ...
      await PUT(
        new Request("http://localhost/api/quality/events/event-1/eight-d", {
          ...
          body: JSON.stringify({
            ...
            problemStatement: "Synthetic problem statement",
            preventionSummary: "Prevent synthetic recurrence",
            systemicChanges: ["Update generic work instruction"],
  ...
  it("advances through the service gate instead of trusting client discipline", async () => {
    mocks.authenticateRequest.mockResolvedValue(auth("QUALITY_MANAGER"));
    const response = requireResponse(
      await PATCH(
        new Request("http://localhost/api/quality/events/event-1/eight-d", {
  ...
  it("maps discipline gate failures to conflict responses", async () => {
    ...
    mocks.advanceEightD.mockRejectedValue(
      new mocks.EightDError(
        "ROOT_CAUSE_REQUIRED",
        "D4 requires confirmed root-cause analysis",
      ),
    );
    const response = requireResponse(
      await PATCH(
