...
describe("notification center", () => {
  ...
  it("uses the latest quality snapshot and ignores closed or low-severity events", async () => {
    mocks.auditFindMany.mockResolvedValue([
      {
        entityId: "qe-1",
        afterJson: qualitySnapshot({ id: "qe-1", eventNumber: "QE-001", severity: "CRITICAL", status: "CLOSED", title: "Closed complaint" }),
      },
      {
        entityId: "qe-1",
        afterJson: qualitySnapshot({ id: "qe-1", eventNumber: "QE-001", severity: "CRITICAL", status: "OPEN", title: "Stale open complaint" }),
      },
      {
        entityId: "qe-2",
        afterJson: qualitySnapshot({ id: "qe-2", eventNumber: "QE-002", severity: "LOW", status: "OPEN", title: "Low observation" }),
      },
    ]);
  ...
  it("limits reorder alerts before composing the final feed", async () => {
    ...
    expect(result.items.filter((item) => item.kind === "REORDER_ALERT")).toHaveLength(NOTIFICATION_REORDER_LIMIT);
  });
