...
describe("GET /api/analytics/pm-compliance", () => {
  it("resolves inclusive calendar dates in the selected site's IANA timezone", async () => {
    const response = await GET(
      request("organizationId=org-a&siteId=site-a&from=2026-07-01&to=2026-08-01&assetId=asset-a"),
    );
    ...
    expect(mocks.buildPmCompliance).toHaveBeenCalledWith({
      ...
      to: new Date("2026-08-01T22:00:00.000Z"),
      assetId: "asset-a",
    });
  ...
  it("rejects missing calendar dates before authentication", async () => {
  ...
  it("rejects impossible calendar dates instead of silently rolling them forward", async () => {
    const response = await GET(
      request("organizationId=org-a&siteId=site-a&from=2026-02-30&to=2026-03-02"),
    );
  ...
  it("rejects users without site access before analytics queries", async () => {
    ...
    const response = await GET(
      request("organizationId=org-a&siteId=site-b&from=2026-07-01&to=2026-08-01"),
    );
  ...
  it("requires maintenance read permission", async () => {
    ...
    const response = await GET(
      request("organizationId=org-a&siteId=site-a&from=2026-07-01&to=2026-08-01"),
    );
