...
describe("atomic calendar rescheduling API", () => {
  ...
  it("writes planning and RESCHEDULED audit inside one transaction", async () => {
    const response = expectResponse(await PATCH(request(), context));
  ...
  it("requires work:manage before reading planning state", async () => {
  ...
  it("rejects cross-site work orders", async () => {
  ...
  it("keeps update and audit rollback-coupled", async () => {
