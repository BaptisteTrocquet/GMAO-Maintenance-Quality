...
function postRequest(filters: Record<string, string> = { due: "OVERDUE" }) {
...
describe("saved views API", () => {
  ...
  it("rejects unsupported filters before calling the service", async () => {
    const response = await POST(postRequest({ status: "COMPLETED" }));
  ...
  it("rejects a site that is not active in the requested organization", async () => {
